...
import { S3Error } from '../service/S3/errors';
...
describe('Express app', () => {
    ...
    describe('POST /vwb', () => {
        ...
        it('should return 200 with the cavatica bulk import response if Authorization header is valid', async () => {
    ...
    describe('GET /vwb/manifest', () => {
        beforeEach(() => {
            (generateManifestPreSignedUrl as jest.Mock).mockReset();
        });

        it('should return 403 if no Authorization header', async () =>
            request(app).get('/vwb/manifest').set('Content-type', 'application/json').expect(403));
        ...
        it('should return 500 if Authorization header is valid but an error occurs', async () => {
            const expectedError = new S3Error('OOPS');
            (generateManifestPreSignedUrl as jest.Mock).mockImplementation(() => {
                throw expectedError;
            });
